...
public class D_CardLayout extends JFrame {
	...
	public D_CardLayout() {
		...
		// 인터페이스 무조건 이벤트를 만들어야함
		// 그래로 따와서 간단하게 구현 추상메소드 구현 필요한 기능만 사용가능 
		
		// EventListener는 특정 이벤트에 대한
		// 모든 감지요소를 가진 인터페이스이다.
		// 이러한 이벤트리스너는 내용을 구현 시
		// 미완성된 메소드 전부를 구현해야하는 불편함이 있다.
		// 따라서 이를 보다 쉽게, 여러 이벤트 중 하나 등을 선택하여
		// 사용하기 위해 이벤트어뎁터를 주로 사용한다.
		card1.addMouseListener(new MouseAdapter() { // 이것만 선택할 때 오버라이드 어떻게 눌렀나
			@Override
		...
		card2.addMouseListener(new MouseAdapter() { // 이것만 선택할 때 오버라이드 어떻게 눌렀나
			@Override
			public void mouseClicked(MouseEvent e) {
				if(e.getButton() == 1){
					// 왼쪽 클릭했느냐!
		...
		card3.addMouseListener(new MouseAdapter() { // 이것만 선택할 때 오버라이드 어떻게 눌렀나
			@Override
			public void mouseClicked(MouseEvent e) {
				if(e.getButton() == 1){
					// 왼쪽 클릭했느냐!
